    ...
    video(video), videoSize(video->pixmap().size()), padding(padding), scale(scale), logoPos(0)
{
    ...
    for(int i = 0; i < 4; i++){
        ...
        connect(rect, &PlaceholderSquare::clicked, [=](){
            moveLogo(i);
        });
    }
}
...
void LogoOverlay::paddingChanged(double newVal)
{
    newVal /= 100;
    this->padding = newVal;
    QPoint points[] = { getTopLeftCorner(true,true), getTopLeftCorner(false,true), getTopLeftCorner(true,false), getTopLeftCorner(false,false)  };
    for(auto& p : points){
        qDebug() << p;
    }
    for(int i = 0; i < 4; i++){
