...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(ui->sizeSlider, &QSlider::valueChanged, [=](int val) {
        ui->sizeSpinBox->setValue(val);
        ui->gv->repaint();
        ui->gv->setVisible(false);
        ui->gv->setVisible(true);
    });
    connect(ui->sizeSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [=](double d) {
        ui->sizeSlider->setValue(round(d));
    });
    connect(ui->paddingSlider, &QSlider::valueChanged, [=](int val) {
        ui->paddingSpinBox->setValue(val / 8.0);
    });
    connect(ui->paddingSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [=](double d) {
        ui->paddingSlider->setValue(round(8 * d));
    });
    connect(ui->opacitySlider, &QSlider::valueChanged, [=](int val) {
        ui->opacitySpinBox->setValue(val);
    });
    connect(ui->opacitySpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), [=](double d) {
    ...
    //Update scale of logo
    connect(ui->paddingSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), overlay,
            &LogoOverlay::paddingChanged);
    //Update opacity
    connect(ui->opacitySpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged), overlay,
    ...
    connect(ui->frame, &DragFrame::videoDropped, this, &MainWindow::setVideo);
...
void MainWindow::watermarkVideo() {
    ...
    ffmpeg->start("/usr/local/bin/ffmpeg", {
            "-i",
            currentVideo,
            "-i",
            currentLogo,
...
void MainWindow::logoChange() {
    ...
    auto picture = QFileDialog::getOpenFileName(this,
